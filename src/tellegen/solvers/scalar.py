    ...
    def forward(ctx, f, lo, hi, tol, max_iter, *params):
        with torch.no_grad():
            ...
            for _ in range(max_iter):
                ...
                # Bracket width alone is the sound convergence test: the bracket always
                # contains the root (by construction from the sign test above), so a narrow
                # bracket guarantees x is within tol of the root regardless of how f behaves
                # near it. |f(x)| < tol is NOT a safe alternative on its own: at a root of
                # multiplicity > 1 (f'(root) == 0, e.g. f(x) = x**3 - c at c = 0), f flattens
                # out near the root, so |f(x)| can already be tiny while x is still far from
                # the root in absolute terms -- this was verified to produce a spurious early
                # exit (x ~ 1e-4 reported as converged at tol 1e-12) before this was pinned to
                # the bracket width only.
                ...
                newton_x = torch.where(dfx != 0, x - fx / dfx, 0.5 * (lo_c + hi_c))
                inside = (newton_x > lo_c) & (newton_x < hi_c) & torch.isfinite(newton_x)
                x = torch.where(inside, newton_x, 0.5 * (lo_c + hi_c))
